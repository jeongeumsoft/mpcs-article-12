...
namespace Exit11\Article\Traits;
...
trait OrderableTrait
{
    public static function bootOrderableTrait()
    {
        static::creating(function ($model) {
            if ($model instanceof Orderable && $model->shouldSortWhenCreating()) {
    ...
    {
        if (
            isset($this->orderable['order_column_name']) &&
            !empty($this->orderable['order_column_name'])
        ) {
            return $this->orderable['order_column_name'];
        }
    ...
    {
        return $this->orderable['sort_when_creating'] ?? true;
    }
    ...
    public function swapOrderWithModel(Orderable $otherModel)
    {
        $orderColumnName = $this->determineOrderColumnName();
    ...
    public static function swapOrder(Orderable $model, Orderable $otherModel)
    {
